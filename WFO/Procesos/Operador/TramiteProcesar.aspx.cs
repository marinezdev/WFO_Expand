    ...
    public partial class TramiteProcesar : Utilerias.Comun
    {
        ...
        private void PintaMesa(int pIdMesa, int IdUsuario)
        {
            ...
            if (mesa.Count > 0)
            {
                ...
                btnRevionMedica.Visible = Convert.ToBoolean((permisosMesaControles = permisosMesa.PermisosMesaControles_Selecionar(pIdMesa, "btnRevionMedica")).Activo);
                ...
                {
                    case "SUSCRIPCIÓN":
                        btnAceptar.Text = "Emitir";
                        break;

                    case "CONTROL":
                        btnAceptar.Text = "Emitir";
                        btnSelccionCompleta.Text = "Suscripción";
                        break; 
        ...
        private void VerificaTramiteDisponible(int pIdMesa)
        {
            ...
            if (Tramite_a_Procesar.Count > 0)
            {
                for (int i = 0; i < Tramite_a_Procesar.Count; i++)
                {
                    ...
                    lblTipoServicioValue.Text = Tramite_a_Procesar[i].TipoServicio;

                    if (Tramite_a_Procesar[i].TipoServicio == "Nuevo Negocio")
                    {
                        LabelNumeroPoliza.Text = "Numero de Solicitud: " ;
                        LabelNumeroPoliza2.Text = Tramite_a_Procesar[i].NumeroOrden?.ToString() ?? "";
                    }
                    else
                    {
                        LabelNumeroPoliza.Text = "Numero de Poliza: ";
                        LabelNumeroPoliza2.Text = Tramite_a_Procesar[i].IdSisLegados?.ToString() ?? "";
                    }
                    LabelNumeroPoliza.Visible = true;
                    ...
                    else
                    {
                        ...
                        for (int p = 0; p < Promotoria.Count; p++)
                        {
                            ...
                            //lblTipoServicio.Text = Tramite_a_Procesar[i].TipoServicio;
        ...
        protected void btnAceptarValida_Click(object sender, EventArgs e)
        {
            ...
            if (txtObservacionesPrivadas.Text.Trim().Length < 5)
            {
                string script = "";
        ...
        {
            List<promotoria.expediente> LstArchExpediente = new List<promotoria.expediente>();
            if (HttpContext.Current.Session["documentos"] != null) 
            {
                LstArchExpediente = (List<promotoria.expediente>)HttpContext.Current.Session["documentos"]; 
            }
            string strDoctoWeb = "";
            string result = "";
            ...
            foreach (promotoria.expediente oArchivo in LstArchExpediente)
            {
                if (oArchivo.Id_Archivo == Id)
                {
                    strDoctoWeb = "..\\..\\" + oArchivo.CarpetaInicial + "\\" + oArchivo.NmArchivo;
                }
            }
        ...
        private string registraDocumentosExpediente(int pIdTramite, int TipoTramite)
        {
            ...
            string directorioTemporal = expediente.CarpetaArchivada;
        ...
        protected void btnSendToMesa_Click(object sender, EventArgs e)
        {
            if (cboToSend.SelectedIndex != -1)
            {
                ...
                string observacionesPub = ""; //txtObservacionesEnviarMesa.Text.ToString().Trim().Replace("'", "");
                string observacionesPrv = txtObservacionesPrivadas.Text.ToString().Trim().Replace("'", "");

                List<Propiedades.Procesos.Operacion.TramiteProcesado> objResultado = Tramites.EnviarTramite(IdTramite, IdMesa, IdMesaToSend, manejo_sesion.Usuarios.IdUsuario, observacionesPub, observacionesPrv);
                if (objResultado[0].IdTramite > 0)
                {
                    // REGISTRO DE ARCHIVOS - COLOCAR DESPUES DE ACTUALIZAR EL TRAMITE
                    if (Session["documentos"] != null) { string resultadoExpediente = registraDocumentosExpediente(IdTramite, TipoTramite); }
        ...
        protected void btnSeleccionEnviarRevionMedica_Clic(object sender, EventArgs e)
        {
            // ### Pendiente: realizar esta actividad de manera proramatica porque de lo contrario no funciona...

            int TipoTramite = int.Parse(hfTipoTramite.Value.ToString());
            int IdTramite = int.Parse(hfIdTramite.Value.ToString());
            int IdMesaToSend = 15;  //int.Parse(cboToSend.SelectedValue.ToString());
            int IdMesa = 13;        //int.Parse(hfIdMesa.Value.ToString());
            ...
            switch (TipoTramite)
            {
                ...
                default:
                    IdMesaToSend = int.Parse(cboToSend.SelectedValue.ToString());
                    IdMesa = int.Parse(hfIdMesa.Value.ToString());
                    break;
            }
        ...
        protected void btnAceptar_Click(object sender, EventArgs e)
        {
            ...
            string observacionesPub = "";
            if (hfNombreMesa.Value.ToString() == "EJECUCIÓN")
            {
                observacionesPub = txtObservacionesPublicas.Text.ToString().Trim().Replace("'", "");

                //observacionesPub = txtObservacionesPublicas.Text.ToString().Trim();
            }


            List<Propiedades.Procesos.Operacion.TramiteProcesado> objResultado = Tramites.ProcesarTramite(IdTramite, int.Parse(hfIdMesa.Value), manejo_sesion.Usuarios.IdUsuario, IdStatusMesaProcesado, observacionesPub, txtObservacionesPrivadas.Text.Trim().Replace("'", ""), "");
            if (objResultado[0].IdTramite > 0)
            {
                int TipoTramite = int.Parse(hfTipoTramite.Value.ToString());
                // REGISTRO DE ARCHIVOS - COLOCAR DESPUES DE ACTUALIZAR EL TRAMITE
        ...
        {            
            ...
            List<Propiedades.Procesos.Operacion.TramiteProcesado> objResultado = Tramites.ProcesarTramite(IdTramite, int.Parse(hfIdMesa.Value), manejo_sesion.Usuarios.IdUsuario, IdStatusMesa, txtObservacionesPublicasHold.Text.Trim().Replace("'", ""), txtObservacionesPrivadas.Text.Trim().Replace("'", ""), "");
        ...
        protected void btnAplicarHold_Click(object sender, EventArgs e)
        {
            ...
            if (nodos.Count > 0)
            {
                if (txtObservacionesPublicasHold.Text.Length > 0)
                {
                    ...
                    List<Propiedades.Procesos.Operacion.TramiteProcesado> objResultado = Tramites.ProcesarTramite(IdTramite, int.Parse(hfIdMesa.Value), manejo_sesion.Usuarios.IdUsuario, IdStatusMesa, txtObservacionesPublicasHold.Text.Trim().Replace("'", ""), txtObservacionesPrivadas.Text.Trim().Replace("'", ""), MotivosRechazos);
                    if (!Page.ClientScript.IsStartupScriptRegistered(this.GetType(), "PopupScript"))
                    {
                        Page.ClientScript.RegisterStartupScript(this.GetType(), "PopupScript", "LimpiarForm();", true);
                    }
        ...
        protected void btnAplicarCancelar_Click(object sender, EventArgs e)
        {
            ...
            if (nodos.Count > 0)
            {
                if (txObservacionesCancelacion.Text.Length > 0)
                {
                    ...
                    List<Propiedades.Procesos.Operacion.TramiteProcesado> objResultado = Tramites.ProcesarTramite(IdTramite, int.Parse(hfIdMesa.Value), manejo_sesion.Usuarios.IdUsuario, IdStatusMesa, txObservacionesCancelacion.Text.Trim().Replace("'", ""), txtObservacionesPrivadas.Text.Trim().Replace("'", ""), MotivosRechazos);
                    if (!Page.ClientScript.IsStartupScriptRegistered(this.GetType(), "PopupScript"))
                    {
                        Page.ClientScript.RegisterStartupScript(this.GetType(), "PopupScript", "LimpiarForm();", true);
                    }
        ...
        protected void btnAplicarSuspender_Click(object sender, EventArgs e)
        {
            ...
            if (hfNombreMesa.Value.ToString() == "EJECUCIÓN")
            {
                txtObservacionesPublicasSuspender.Text = txtObservacionesPublicas.Text;
            }
            ...
            if (nodos.Count > 0)
            {
                if (txtObservacionesPublicasSuspender.Text.Length > 0)
                {
                    ...
                    List<Propiedades.Procesos.Operacion.TramiteProcesado> objResultado = Tramites.ProcesarTramite(IdTramite, int.Parse(hfIdMesa.Value), manejo_sesion.Usuarios.IdUsuario, IdStatusMesa, txtObservacionesPublicasSuspender.Text.Trim().Replace("'", ""), txtObservacionesPrivadas.Text.Trim().Replace("'", ""), MotivosRechazos);
                    if (!Page.ClientScript.IsStartupScriptRegistered(this.GetType(), "PopupScript"))
                    {
                        Page.ClientScript.RegisterStartupScript(this.GetType(), "PopupScript", "LimpiarForm();", true);
                    }
        ...
        protected void btnCtrlPausarTramite_Click(object sender, EventArgs e)
        {
            ...
            List<Propiedades.Procesos.Operacion.TramiteProcesado> objResultado = Tramites.ProcesarTramite(IdTramite, int.Parse(hfIdMesa.Value), manejo_sesion.Usuarios.IdUsuario, IdStatusMesa, txtObservacionesPublicasPausar.Text.Trim().Replace("'", ""), txtObservacionesPrivadas.Text.Trim().Replace("'", ""), "");
            if (!Page.ClientScript.IsStartupScriptRegistered(this.GetType(), "PopupScript"))
            {
                Page.ClientScript.RegisterStartupScript(this.GetType(), "PopupScript", "LimpiarForm();", true);
            }
        ...
        protected void Rechazo(string MotivosRechazos)
        {
            if (hfNombreMesa.Value.ToString() == "EJECUCIÓN")
            {
                txtObservacionesPublicasRechazara.Text = txtObservacionesPublicas.Text;
            }
            ...
            if (txtObservacionesPublicasRechazara.Text.Length > 0)
            {
                ...
                List<Propiedades.Procesos.Operacion.TramiteProcesado> objResultado = Tramites.ProcesarTramite(IdTramite, int.Parse(hfIdMesa.Value), manejo_sesion.Usuarios.IdUsuario, IdStatusMesa, txtObservacionesPublicasRechazara.Text.Trim().Replace("'", ""), txtObservacionesPrivadas.Text.Trim().Replace("'", ""), MotivosRechazos);
                if (!Page.ClientScript.IsStartupScriptRegistered(this.GetType(), "PopupScript"))
                {
                    Page.ClientScript.RegisterStartupScript(this.GetType(), "PopupScript", "LimpiarForm();", true);
                }
        ...
        private string registraDocExp(int pIdTramite, string directorioCarta, int TipoTramite)
        {
            ...
            string directorioInicial = Server.MapPath("~") + expediente.CarpetaInicial;
            string directorioArchivos = expediente.CarpetaArchivada;
            //string directorioTemporal = Server.MapPath("~") + "\\DocsUp\\";
            ...
            if (expedientes.Count > 0)
            {
                ArchFusionAnt = directorioArchivos + expedientes[0].NmArchivo;
                Id_Expediente = expedientes[0].Id;
            }

            List<promotoria.control_archivos> control_Archivos = archivos.ControlArchivoNuevoID();
            int IdControlArchivo = control_Archivos[0].Id;
            string nombreFusion = directorioInicial + control_Archivos[0].Clave + ".pdf";
            ...
            manejo_sesion = (WFO.IU.ManejadorSesion)Session["Sesion"];
            string nmSeparador = directorioInicial + manejo_sesion.Usuarios.IdUsuario + ".pdf";
            ...
            if (string.IsNullOrEmpty(msgError))
            {
                ...
                File.Copy(nombreFusion, directorioArchivos + control_Archivos[0].Clave + ".pdf");
                //archivos.Agregar_Expedientes_Tramite(TipoTramite, pIdTramite, IdControlArchivo, IdControlArchivo.ToString().PadLeft(12, '0') + ".pdf", "Archivo Fucion Agrgacion", 1, 1, "");
                msgError = "";
            }
            else
