...
{
    public partial class NuevoPamDigital : Utilerias.Comun
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                listaPromotoriaZona(manejo_sesion.Usuarios.IdUsuario);
        ...
        private void ListaTipoServicio(int TipoTramite)
        {
            List<prop.cat_tipo_servicio> cat_tipo_servicio = Catalogos.cat_tipo_servicio_Seleccionar_PorTipo_tramite(TipoTramite);
            LisTipoServicio.DataSource = cat_tipo_servicio;
            LisTipoServicio.DataBind();
            LisTipoServicio.DataTextField = "Nombre";
            LisTipoServicio.DataValueField = "Id";
            LisTipoServicio.DataBind();
        }

        private void listaPromotoriaZona(int idUsuario)
        {
            List<prop.cat_PromotoriaZonas> cat_PromotoriaZonas = Catalogos.cat_PromotoriaZonas_Seleccionar(idUsuario);
            cboPromotoriaZona.DataSource = cat_PromotoriaZonas;
            cboPromotoriaZona.DataBind();
            cboPromotoriaZona.DataTextField = "Clave";
            cboPromotoriaZona.DataValueField = "Id";
            cboPromotoriaZona.DataBind();
            cboPromotoriaZona.Enabled = true;

            if (cboPromotoriaZona.Items.Count == 2) 
            {
                cboPromotoriaZona.SelectedIndex = 1;
                cboPromotoriaZona.Enabled = false;
            }
        }
        ...
        protected void LisTipoServicio_SelectedIndexChanged(object sender, EventArgs e)
        {
            // ### Pendiente: todos los trámites van a call center
            if (LisTipoServicio.SelectedItem.ToString() == "NUEVO NEGOCIO")
            {
                lblNumPolizaSolicitud.Text = "Número de solicitud";
                lblNumPolizaSolicitud.Font.Bold = false;
                //txtTelefono1.Text = "";
                //txtTelefono2.Text = "";
            ...
            {
                lblNumPolizaSolicitud.Text = "* Número de póliza";
                lblNumPolizaSolicitud.Font.Bold = true;
                //txtTelefono1.Text = "0000000000";
                //txtTelefono2.Text = "0000000000";
        ...
        protected void BtnContinuar_Click(object sender, EventArgs e)
        {
            bool _error = false;
            string script = "";
            int tel1len = txtTelefono1.Text.Trim().Length;
            int tel2len = txtTelefono2.Text.Trim().Length;


            if (tel1len != 10)
            {
                script = "AlertaTelefonos();";
            ...
            if (LisTipoServicio.SelectedItem.ToString() != "NUEVO NEGOCIO")
            {
                string numeroPoliza = textNumeroOrden.Text.ToString().Trim();
                if (numeroPoliza.Length != 6)
                {
                    script = "AlertaNumeroPoliza();";
                    ScriptManager.RegisterStartupScript(this, GetType(), "ServerControlScript", script, true);
                    _error = true;
                }
            }
        ...
        private void PostBack(prop.TramiteN1 oEmisionVidaGMM)
        {
            ...
            /***********************************************************************/
            LisTipoServicio.SelectedValue = oEmisionVidaGMM.IdTipoServicio.ToString();
        ...
        private prop.TramiteN1 recuperaCaptura()
        {
            ...
            NuevoTramite.IdTipoServicio = Convert.ToInt32(LisTipoServicio.SelectedValue);
            NuevoTramite.IdPromotoriaZona = Convert.ToInt32(cboPromotoriaZona.SelectedValue);
            NuevoTramite.Telefono1 = txtTelefono1.Text.Trim();
            NuevoTramite.Telefono2 = txtTelefono2.Text.Trim();
            ...
            try
            {
                NuevoTramite.PrimaTotal = txtPrimaTotal.Text.Trim().Replace(",","");
                NuevoTramite.PrimaExcedente = txtPrimaExcedente.Text.Trim().Replace(",", "");
                NuevoTramite.PrimaDiferencia = (double.Parse(NuevoTramite.PrimaTotal) - double.Parse(NuevoTramite.PrimaExcedente)).ToString();
            }
            catch (Exception ex)
            {
        ...
        protected void txtPrimaTotal_TextChanged(object sender, EventArgs e)
        {
            try
            {
                txtPrimaTotal.Text = txtPrimaTotal.Text.Trim().Replace(",", "");
                txtPrimaTotal.Text = string.Format("{0:#,##0.00}", Convert.ToDecimal(txtPrimaTotal.Text));
            }
            catch
            { 
                // Ignoramos errores
        ...
        protected void txtPrimaExcedente_TextChanged(object sender, EventArgs e)
        {
            try
            {
                txtPrimaExcedente.Text = txtPrimaExcedente.Text.Trim().Replace(",", "");
                txtPrimaExcedente.Text = string.Format("{0:#,##0.00}", Convert.ToDecimal(txtPrimaExcedente.Text));
            }
            catch
            {
                // Ignoramos errores
